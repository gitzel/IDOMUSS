  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<String>>(
        ...
        builder: (context, snapshot) {
          ...
          snapshot.data.sort();
          return CadastroScaffold(<Widget>[
            BackButton(
                ...
                children: <Widget>[
                    ...
                    child: RangeSlider(
                      ...
                      onChanged: (values) {
                        setState(() {
                          rangeValues = values;
                          rangeLabels = RangeLabels(
                              "R\$${values.start.toInt().toString()}",
                              "R\$${values.end.toInt().toString()}");
                          minimo = TextEditingController.fromValue(
                              new TextEditingValue(
                                  text: values.start.toInt().toString()));
                          maximo = TextEditingController.fromValue(
                              new TextEditingValue(
                  ...
                  Padding(
                    ...
                    child: TextFieldOutline(
                      ...
                      validator: (val) =>
                          val.length > 0 ? null : "Nome inválido!",
                      onChanged: (val) {
                        setState(() {
                          rangeValues = RangeValues(
                              double.parse(val.toString()), rangeValues.end);
                        });
                      },
                      controller: minimo,
                    ),
                  ...
                  Padding(
                    ...
                    child: TextFieldOutline(
                      ...
                      keyboardType: TextInputType.number,
                      validator: (val) =>
                          int.parse(val) >= 0 ? null : "Numéro inválido!",
                      onChanged: (val) {
                        setState(() {
                          if (int.parse(val) > rangeValues.start)
                            rangeValues = RangeValues(rangeValues.start,
